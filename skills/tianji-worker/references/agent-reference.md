...
This page is a machine-oriented reference for coding agents that generate Tianji Worker source code. For the user-facing setup and operation guide, read [Worker Getting Started](https://tianji.dev/docs/worker/getting-started).

For an installable bundle that also covers common operations, [download the Tianji Worker Skill](https://tianji.dev/docs/worker/agent-skill).
...
The [Worker Getting Started guide](https://tianji.dev/docs/worker/getting-started#ask-an-agent-to-write-your-worker) provides a ready-to-copy request template and the complete user workflow.
...
Workers can use static imports from published workspace modules with aliases such as `@shared/math`. Inspect the module's exports and declarations in the dashboard before importing it; do not invent aliases or exported functions.

Each binding pins a specific module revision. Saving or testing code with an existing import keeps its current pin; a newly added alias resolves to the latest published revision when bindings are not supplied explicitly. Updating a shared module does not automatically upgrade an existing Worker's pin. A Worker can bind at most 16 shared modules, and shared modules cannot import dependencies themselves.

Use the Worker editor to inspect bindings and test the code together with its selected revisions. A plain dashboard Worker cannot resolve arbitrary npm imports; CLI dependencies must be bundled and compatible with the sandbox.
